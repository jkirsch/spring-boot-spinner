var Spinner = function(el, cfg) {
    ...
    var init = function() {        
        vis = d3.select(element)
                ...
                .attr("transform", "translate(" + (config.outerR + config.margins.left) + "," + (config.outerR/2 + config.margins.top) + ")") //move the center of the pie chart from 0, 0 to radius, radius
